...
export default defineTool({
	...
	handler: async (input: { start_date: string; end_date: string }, ctx: ExecutionContext) => {
		const query = `FROM sales SHOW orders, returns, gross_sales, net_sales, discounts SINCE ${input.start_date} UNTIL ${input.end_date}`;
		const result = await executeShopifyQL(query, ctx);

		const row = result.data[0] ?? {};
		const totalOrders = (row.orders as number) ?? 0;
		const returnedOrders = (row.returns as number) ?? 0;
		const grossSales = (row.gross_sales as number) ?? 0;
		const netSales = (row.net_sales as number) ?? 0;
		const discountsVal = (row.discounts as number) ?? 0;
		// Refund amount ≈ gross_sales - net_sales - abs(discounts)
		const totalRefundAmount = Math.round(Math.max(0, grossSales - netSales - Math.abs(discountsVal)) * 100) / 100;
		const refundRate = totalOrders > 0 ? Math.round((returnedOrders / totalOrders) * 10000) / 100 : 0;

		return { totalOrders, returnedOrders, refundRate, totalRefundAmount };
	},
});
